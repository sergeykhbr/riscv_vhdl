...
void sdctrl_regs::generateVCD(sc_trace_file *i_vcd, sc_trace_file *o_vcd) {
    ...
    if (o_vcd) {
        ...
        sc_trace(o_vcd, r.sclk_ena, pn + ".r.sclk_ena");
        sc_trace(o_vcd, r.spi_mode, pn + ".r.spi_mode");
        sc_trace(o_vcd, r.err_clear, pn + ".r.err_clear");
        sc_trace(o_vcd, r.scaler_400khz, pn + ".r.scaler_400khz");
        sc_trace(o_vcd, r.scaler_data, pn + ".r.scaler_data");
        ...
        sc_trace(o_vcd, r.last_req_cmd, pn + ".r.last_req_cmd");
        sc_trace(o_vcd, r.last_resp_cmd, pn + ".r.last_resp_cmd");
        sc_trace(o_vcd, r.last_resp_crc7_rx, pn + ".r.last_resp_crc7_rx");
        sc_trace(o_vcd, r.last_resp_crc7_calc, pn + ".r.last_resp_crc7_calc");
        sc_trace(o_vcd, r.last_resp_reg, pn + ".r.last_resp_reg");
        sc_trace(o_vcd, r.pcie_12V_support, pn + ".r.pcie_12V_support");
        sc_trace(o_vcd, r.pcie_available, pn + ".r.pcie_available");
        sc_trace(o_vcd, r.voltage_supply, pn + ".r.voltage_supply");
        sc_trace(o_vcd, r.check_pattern, pn + ".r.check_pattern");
    }
...
void sdctrl_regs::comb() {
    ...
    if (i_cmd_req_valid.read() == 1) {
        v.last_req_cmd = i_cmd_req_cmd.read();
    }
    if (i_cmd_resp_valid.read() == 1) {
        v.last_resp_cmd = i_cmd_resp_cmd.read();
        v.last_resp_crc7_rx = i_cmd_resp_crc7_rx.read();
        v.last_resp_crc7_calc = i_cmd_resp_crc7_calc.read();
        v.last_resp_reg = i_cmd_resp_reg.read();
    }
    ...
    case 0x004:                                             // {0x10, 'RO', 'status', 'state machines status'}
        vb_rdata(3, 0) = i_err_code.read();                 // the latest error code
        vb_rdata(14, 12) = i_sdtype.read();                 // detected card type
        break;
    case 0x005:                                             // {0x14, 'RO', 'last_cmd_response', 'Last CMD response data'}
        vb_rdata(5, 0) = r.last_req_cmd.read();
        vb_rdata(13, 8) = r.last_resp_cmd.read();
        vb_rdata(22, 16) = r.last_resp_crc7_rx.read();
        vb_rdata(30, 24) = r.last_resp_crc7_calc.read();
        break;
    case 0x006:                                             // {0x18, 'RO', 'last_cmd_resp_arg'}
        vb_rdata = r.last_resp_reg.read();
        break;
    case 0x008:                                             // {0x20, 'RW', 'interface_condition', 'CMD8 parameters'}
        vb_rdata(7, 0) = r.check_pattern.read();
        vb_rdata(11, 8) = r.voltage_supply.read();
    ...
    o_pcie_12V_support = r.pcie_12V_support.read();
    o_pcie_available = r.pcie_available.read();
    o_voltage_supply = r.voltage_supply.read();
    o_check_pattern = r.check_pattern.read();

    o_sck = r.level.read();
    o_sck_posedge = v_posedge;
    o_sck_negedge = v_negedge;
    o_watchdog = r.wdog.read();
