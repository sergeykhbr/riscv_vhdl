...
void sdctrl_cmd_transmitter::generateVCD(sc_trace_file *i_vcd, sc_trace_file *o_vcd) {
    ...
    if (o_vcd) {
        ...
        sc_trace(o_vcd, r.req_cmd, pn + ".r.req_cmd");
        sc_trace(o_vcd, r.req_rn, pn + ".r.req_rn");
        sc_trace(o_vcd, r.resp_valid, pn + ".r.resp_valid");
        sc_trace(o_vcd, r.resp_cmd, pn + ".r.resp_cmd");
        sc_trace(o_vcd, r.resp_arg, pn + ".r.resp_arg");
        sc_trace(o_vcd, r.resp_spistatus, pn + ".r.resp_spistatus");
        sc_trace(o_vcd, r.cmdshift, pn + ".r.cmdshift");
        sc_trace(o_vcd, r.cmdmirror, pn + ".r.cmdmirror");
        sc_trace(o_vcd, r.regshift, pn + ".r.regshift");
        sc_trace(o_vcd, r.cidshift, pn + ".r.cidshift");
        sc_trace(o_vcd, r.crc_calc, pn + ".r.crc_calc");
        sc_trace(o_vcd, r.crc_rx, pn + ".r.crc_rx");
        sc_trace(o_vcd, r.cmdbitcnt, pn + ".r.cmdbitcnt");
        sc_trace(o_vcd, r.crc7_clear, pn + ".r.crc7_clear");
        sc_trace(o_vcd, r.cmdstate, pn + ".r.cmdstate");
        sc_trace(o_vcd, r.err_valid, pn + ".r.err_valid");
        sc_trace(o_vcd, r.err_setcode, pn + ".r.err_setcode");
        sc_trace(o_vcd, r.cmd_cs, pn + ".r.cmd_cs");
...
void sdctrl_cmd_transmitter::comb() {
    ...
    vb_cmdshift = r.cmdshift.read();
    vb_resp_spistatus = r.resp_spistatus.read();
    v.err_valid = 0;
    v.err_setcode = 0;
    ...
    if (i_sclk_negedge.read() == 1) {
        ...
        if (r.cmdstate.read() == CMDSTATE_IDLE) {
            ...
            if (i_err_code.read() != CMDERR_NONE) {
                ...
                v.cmd_cs = 0;
                v.req_cmd = i_req_cmd.read();
                v.req_rn = i_req_rn.read();
    ...
    o_cmd_cs = r.cmd_cs.read();
    o_req_ready = v_req_ready;
    o_resp_valid = r.resp_valid.read();
    o_resp_cmd = r.cmdmirror.read();
    o_resp_reg = r.regshift.read();
    o_resp_crc7_rx = r.crc_rx.read();
    o_resp_crc7_calc = r.crc_calc.read();
    o_resp_spistatus = r.resp_spistatus.read();
    o_wdog_ena = r.wdog_ena.read();
    o_err_valid = r.err_valid.read();
    o_err_setcode = r.err_setcode.read();
}
