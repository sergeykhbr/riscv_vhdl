...
SC_MODULE(lrunway) {
 ...
 private:
    ...
    sc_uint<LINE_WIDTH> wb_tbl_rdata;   // no need to signal mem_update used instead
    sc_signal<sc_uint<LINE_WIDTH>> wb_tbl_wdata;
    sc_signal<bool> w_we;
    sc_signal<bool> mem_updated;        // systemc only
};
...
void lrunway<abits, waybits>::generateVCD(sc_trace_file *i_vcd,
    ...
    if (o_vcd) {
        ...
        sc_trace(o_vcd, i_raddr, i_raddr.name());
...
void lrunway<abits, waybits>::comb() {
    ...
    wb_tbl_rdata = tbl[radr.read().to_int()];
    ...
    if (wb_tbl_rdata(LINE_WIDTH-1, LINE_WIDTH-waybits) != i_lru.read()) {
        ...
        for (int i = WAYS_TOTAL-2; i >= 0; i--) {
            if (shift_ena_up == 1) {
                vb_tbl_wdata_up((i+1)*waybits-1, i*waybits) =
                        wb_tbl_rdata((i+2)*waybits-1, (i+1)*waybits);
                if (wb_tbl_rdata((i+1)*waybits-1, i*waybits) == i_lru.read()) {
                    shift_ena_up = 0;
                }
            }
        }
        ...
        for (int i = 1; i < WAYS_TOTAL; i++) {
            if (shift_ena_down == 1) {
                vb_tbl_wdata_down((i+1)*waybits-1, i*waybits) =
                        wb_tbl_rdata(i*waybits-1, (i-1)*waybits);
                if (wb_tbl_rdata((i+1)*waybits-1, i*waybits) == i_lru.read()) {
                    shift_ena_down = 0;
                }
            }
...
void lrunway<abits, waybits>::registers() {
    radr = i_raddr.read();
    if (w_we.read() == 1) {
        tbl[i_waddr.read().to_int()] = wb_tbl_wdata;
        mem_updated = !mem_updated.read();
    }
}
