...
Processor::Processor(sc_module_name name_, uint32_t hartid, bool async_reset,
    ...
    csr0->i_sp(ireg.sp);
    csr0->i_req_valid(w.e.csr_req_valid);
    csr0->o_req_ready(csr.req_ready);
    csr0->i_req_type(w.e.csr_req_type);
    csr0->i_req_addr(w.e.csr_req_addr);
    csr0->i_req_data(w.e.csr_req_data);
    csr0->o_resp_valid(csr.resp_valid);
    csr0->i_resp_ready(w.e.csr_resp_ready);
