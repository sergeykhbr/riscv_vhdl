...
const AttributeType &AttributeType::operator[](unsigned idx) const {
    if (is_list()) {
        ...
        LIB_printf("%s", "Non-indexed attribute type");
    }
    return NilAttribute;
}
...
const uint8_t &AttributeType::operator()(unsigned idx) const {
    if (idx > size()) {
        LIB_printf("Data index '%d' out of range.", idx);
        return u_.data[0];
    }
    if (size_ > 8) {
        return u_.data[idx];
...
void AttributeType::insert_to_list(unsigned idx, const AttributeType *item) {
    if (idx > size_) {
        LIB_printf("%s", "Insert index out of bound");
        return;
    }
...
void AttributeType::remove_from_list(unsigned idx) {
    if (idx >= size_) {
        LIB_printf("%s", "Remove index out of range");
        return;
    }
    (*this)[idx].attr_free();
    if (idx == (size() - 1)) {
...
void AttributeType::sort(int idx) {
    if (!is_list()) {
        LIB_printf("%s", "Sort algorithm can applied only to list attribute");
    }
...
void attribute_to_string(const AttributeType *attr, AutoBuffer *buf) {
    if (attr->is_nil()) {
        ...
        LIB_printf("Not implemented interface to dict. method");
    } else if (attr->is_floating()) {
        char fstr[64];
        LIB_sprintf(fstr, sizeof(fstr), "%.4f", attr->to_float());
...
int string_to_attribute(const char *cfg, int &off,
    ...
    if (cfg[off] == '\'' || cfg[off] == '"') {
        ...
        off += str_sz;
        if (cfg[off] != t1) {
            LIB_printf("%s", "JSON parser error: Wrong string format");
            out->attr_free();
            return -1;
        }
        ...
        if (cfg[off] != ']') {
            LIB_printf("JSON parser error: Wrong list format");
            out->attr_free();
            return -1;
        }
        off = skip_special_symbols(cfg, off + 1);
        ...
        while (cfg[off] != '}' && cfg[off] != '\0') {
            if (string_to_attribute(cfg, off, &new_key)) {
                LIB_printf("%s", "JSON error: Wrong dictionary key");
                out->attr_free();
                return -1;
            }
            off = skip_special_symbols(cfg, off);
            if (cfg[off] != ':') {
                out->attr_free();
                LIB_printf("%s", "JSON error: Wrong dictionary delimiter");
                return -1;
            }
            off = skip_special_symbols(cfg, off + 1);
            if (string_to_attribute(cfg, off, &new_value)) {
                LIB_printf("%s", "JSON error: Wrong dictionary value");
                out->attr_free();
                return -1;
            }
        ...
        if (cfg[off] != '}') {
            LIB_printf("JSON error: %s", "Wrong dictionary format");
            out->attr_free();
            return -1;
        }
        off = skip_special_symbols(cfg, off + 1);
        ...
        if (out->has_key("Type")) {
            LIB_printf("%s", "Not implemented string to dict. attribute");
        }
    } else if (cfg[off] == '(') {
        AutoBuffer buf;
        char byte_value;
        ...
        while (cfg[off] != ')' && cfg[off] != '\0') {
            ...
            if (cfg[off] != ',') {
                LIB_printf("JSON parser error: Wrong data dytes delimiter");
                out->attr_free();
                return -1;
            }
            off = skip_special_symbols(cfg, off + 1);
        ...
        if (cfg[off] != ')') {
            LIB_printf("JSON parser error: Wrong data format");
            out->attr_free();
            return -1;
        }
    ...
    if (off == checkstart) {
        LIB_printf("JSON parser error: Can't detect format");
        out->attr_free();
        return -1;
    }
    return 0;
